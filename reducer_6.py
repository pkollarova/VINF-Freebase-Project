...
'''
This reducer is used to process sorted data from the sixth mapper.
It does line by line and puts information about all the specific artists awards together into one line.
'''
...
for read_line in input_stream:
    ...
    # If we found some information about our artists award (we stil operate with the same ID that belongs to some artist)
    ...
    # If we did not find the data to the artist we wanted
        ...
        if len(data_list) > 0:
            print(str(current_id) + "\thas_award\t" + " ~~~ ".join(data_list))
        current_id = left_col
        data_list = list()
        data_list.append(right_col)
        continue    
