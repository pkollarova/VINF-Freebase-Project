...
'''
This reducer is used to process sorted data from the first mapper.
It does line by line and puts information about specific artist together into a chunk.
'''
...
for read_line in input_stream:
    ...
    # Check if there is an artist on this line
    # If yes, print it out
    ...
    if current_id == left_col:
        # If we did not find the relation between artist and data we were looking for
    ...
    # If we did not find the data we wanted (normal case)
        ...
        # If the ID we are collecting data for is different but still is it some kind of artist
        if center_col == "an_artist":
            current_id = left_col
            print(left_col + "\tan_artist")
            current_relation = None
